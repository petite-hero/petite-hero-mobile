  ...
  return ([
    ...
    props.substatus === "TYPE" ?
      <Animated.View key={0} style={[styles.locSettingPanel, {left: props.animLeft}]}>
        <Text style={{marginTop: 20, marginLeft: 10, marginBottom: 10, fontWeight: "bold", fontSize: 16}}>Marked as</Text>
        {["Home", "Education"].map((type, index) => {
          return (
            <TouchableOpacity key={index} style={styles.txtTypeContainer} onPress={() => props.onTypeEntrySelected(type)}>
              <Text style={{flex: 8, fontWeight: props.lTypeTmp === type ? "bold" : "normal", color: props.lTypeTmp === type ? COLORS.STRONG_ORANGE : "black"}}>{type}</Text>
              {props.lTypeTmp === type ?
        ...
        {WEEKDAYS.map((day, index) => {
          return (
            <TouchableOpacity key={index} style={styles.txtRepeatDayContainer} onPress={() => props.onRepeatEntrySelected(index)}>
              <Text style={{flex: 8, fontWeight: props.lRepeatTmp[index] ? "bold" : "normal", color: props.lRepeatTmp[index] ? COLORS.STRONG_ORANGE : "black"}}>
                Every {day}
              </Text>
